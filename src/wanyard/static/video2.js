...
class StageZoom {
  constructor(stage, layer, resetButton, canInteract = () => true) {
    this.stage = stage;
    this.layer = layer;
    this.resetButton = resetButton;
    this.canInteract = canInteract;
    this.math = window.WanyardStageZoom;
    this.value = this.math.resetState();
  ...
  set(next) {
    ...
    this.value = this.math.clampState(next, width, height);
    this.apply();
  }
  ...
  zoomBy(factor, point = null) {
    ...
    this.value = this.math.zoomAt(this.value, factor, anchor, width, height);
    this.apply();
  }
  ...
  onPointerMove(evt) {
    ...
    if (this.pointers.size >= 2 && this.pinch) {
      ...
      const factor = this.math.pinchFactor(this.pinch.distance, distance);
      const { width, height } = this.size();
      const zoomed = this.math.zoomAt(this.pinch.state, factor, this.pinch.midpoint, width, height);
      this.set({
        ...zoomed,
...
document.addEventListener("fullscreenchange", () => stageZoom.reset());
...
document.addEventListener("keydown", e => {
  ...
  if (e.key === "+" || e.key === "=") {
    e.preventDefault();
    stageZoom.zoomBy(1.4);
  }
  if (e.key === "-") {
    e.preventDefault();
    stageZoom.zoomBy(1 / 1.4);
  }
  if (e.key === "0") {
    e.preventDefault();
    stageZoom.reset();
  ...
  if (e.key === "Escape"
      && !st.zoneEdit.active
      && !st.activityOpen
      && !st.notificationsOpen
      && (!el.zoneMenu || el.zoneMenu.hidden)) {
    stageZoom.reset();
  }
...
function stageVideoClick(e) {
  if (stageZoom.consumeJustPanned()) {
    e.preventDefault();
...
el.video.addEventListener("click",    stageVideoClick);
el.liveVideo.addEventListener("click", stageVideoClick);
...
function eventToCanvas(evt, c) {
  const box = c.getBoundingClientRect();
  return {
    x: (evt.clientX - box.left) * ((c.clientWidth || 1) / (box.width || 1)),
    y: (evt.clientY - box.top) * ((c.clientHeight || 1) / (box.height || 1)),
  };
}
...
function zonePointAt(evt) {
  ...
  const { x, y } = eventToCanvas(evt, c);
  const box = c.getBoundingClientRect();
  let best = null, bestDist = 12 * ((c.clientWidth || 1) / (box.width || 1));
...
function zoneEdgeAt(evt, maxDist = Infinity) {
  ...
  const c = el.zoneCanvas;
  const box = c.getBoundingClientRect();
  const p = eventToCanvas(evt, c);
  const visualToCanvas = (c.clientWidth || 1) / (box.width || 1);
  let best = null, bestDist = Number.isFinite(maxDist) ? maxDist * visualToCanvas : maxDist;
